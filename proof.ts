import { BatchDBOp, Trie } from "@ethereumjs/trie";
...
const proof = async () => {
  ...
  console.log({ proof });
  console.log(
    "VERIFY inclusion using the proof and root With zero knowledge of the rest of the Trie"
  );
  console.log("-- A valid proof will return the value");
  const newTrie = new Trie({});
  const validate = await newTrie.verifyProof(
    trie.root(),
    fillData[9][0],
    proof
  );
  ...
  try {
    ...
    console.log({
      ...
      validated: (err as any).message,
    });
  }
  const nonKey = Buffer.from(keccak256(Buffer.from("Testkey111")));
...
const proofExample = async () => {
  ...
  const batch: BatchDBOp[] = fillData.map(([k, v]) => {
    return {
      type: "put",
      key: k,
      value: v,
